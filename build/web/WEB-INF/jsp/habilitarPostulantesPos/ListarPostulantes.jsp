...
<table>
  <tr>
    <td>
     <form name="fvolver" action="<c:url value='/habilitarPstPos/entrada.fautapo'/>" method="post">
...
<form name="forma" action="<c:url value="/habilitarPstPos/mostrarPostulantes.fautapo"/>" method="POST">
  <table class="tabla">
  <tr>
  ...
  <c:forEach var="lista" items="${listaPostulantes}" varStatus="contador">
    <tr <c:if test="${(contador.count mod 2) == 0}">bgColor="#FFFFD9" %-- Est&acute;tico :( --%</c:if> onMouseOver="this.className='sobreFila'" onmouseout="this.className=''">
      <td><c:if test="${lista.id_estado == 'P'}"> <input type="checkbox" name="id_postulante_hab"  id="id_postulante_hab<c:out value='${lista.id_postulante}'/>"  value="<c:out value='${lista.id_postulante}'/>"  >
		 ...
		 <c:if test="${lista.id_estado == 'R' && (lista.id_tipo_admision != '1' && lista.id_tipo_admision != '2')}"> <input type="checkbox" name="id_postulante_hab"  id="id_postulante_hab<c:out value='${lista.id_postulante}'/>"  value="<c:out value='${lista.id_postulante}'/>"  >
         </c:if>
		 </td> 
