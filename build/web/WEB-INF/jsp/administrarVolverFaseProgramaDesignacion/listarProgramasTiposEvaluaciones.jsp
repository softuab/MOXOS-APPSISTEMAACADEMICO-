...
<form name='forma' method=post action='<c:url value="/retrocederFaseDesignacion/listarDocentesAsignadosMaterias.fautapo"/>'>
    ...
    <tr>
      <td class="etiqueta">
        <c:choose>
          ...
          <c:when test="${cliente.id_departamento > 0}">Programa</c:when>
        </c:choose>      
      </td>
      <td class="etiqueta">::</td>
...
function verificar(){
  if((document.forma.id_facultad.value!=-1)&&(document.forma.id_programa.value!=-1)&&(document.forma.id_prg_plan.value!=-1)&&(document.forma.id_tipo_evaluacion.value!="")){
