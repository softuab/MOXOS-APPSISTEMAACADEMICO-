                            ...
                            <tr class="filter">
                                ...
                                <td>
                                    <c:if test="${lista.id_estado == 'A'}">
                                        <form id="fnuevo<c:out value='${contador.count}'/>" name="fnuevo<c:out value='${contador.count}'/>" action="<c:url value='/postulantes/verDatosPostulante.fautapo'/>" method="post">
                                            <input type="hidden" name="id_postulante" value="<c:out value='${lista.id_postulante}'/>">
                                            <div> <a class="agregar" href="javascript:enviarSolicitud('fnuevo<c:out value='${contador.count}'/>');"> Certificado Habilitaci&oacute;n</a> </div>
                                        </form>
                                    </c:if>
                                    <c:if test="${(lista.id_estado == 'P') &&(lista.id_tipo_admision > 2 )}">
                                        <span style="color: red"> No Habilitado</span> <br> 
                                        <form id="fsolicitud<c:out value='${contador.count}'/>" name="fsolicitud<c:out value='${contador.count}'/>" action="<c:url value='/postulantes/verDatosPostulante.fautapo'/>" method="post">
                                            <input type="hidden" name="id_postulante" value="<c:out value='${lista.id_postulante}'/>">
                                            <div> <a class="agregar" href="javascript:enviarSolicitud('fsolicitud<c:out value='${contador.count}'/>');"> Solicitud</a> </div>
                                        </form>
                                    </c:if>
                                    <c:if test="${(lista.id_estado == 'E') &&(lista.id_tipo_admision > 2 )}">
                                        <span style="color: red">Inscrito</span> <br> 
                                        <form id="fsolicitud2_<c:out value='${contador.count}'/>" name="fsolicitud2_<c:out value='${contador.count}'/>" action="<c:url value='/postulantes/verDatosPostulante.fautapo'/>" method="post">
                                            <input type="hidden" name="id_postulante" value="<c:out value='${lista.id_postulante}'/>">
                                            <div> <a class="agregar"  href="javascript:enviarSolicitud('fsolicitud2_<c:out value='${contador.count}'/>');"> Solicitud</a> </div>
                                        </form>
                                    </c:if>
                                    <c:if test="${lista.id_estado == 'P'}">
                                        <span style="color: red">No Habilitado</span>
                                    </c:if>
                                </td>
                            </tr>
                        </c:forEach>
