<%@ include file="../../Superior.jsp" %>
  ...
  <table border="0" width="97%" class="tabla">
    <tr>
      ...
      <td>
        <c:out value="${datosAsignacion.gestion}" />
      </td>
      <th align="right">PERIODO :</th>
      <td>
        <c:out value="${datosAsignacion.periodo}" />
      </td>
    </tr>
    <tr>
      <th align="right">ASIGNATURA :</th>
      <td>
        <c:out value="${datosAsignacion.sigla}" /> -
        <c:out value="${datosAsignacion.materia}" />
      </td>
      <th align="right">TIPO EVALUACION :</th>
      <td>
        <c:out value="${datosAsignacion.tipo_evaluacion}" />
      </td>
    ...
    <tr>
      ...
      <td>
        <c:out value="${datosAsignacion.nivel_academico}" />
      </td>
      <th align="right"> </th>
      <td></td>
      <th align="right"></th>
  ...
  <form name=forma action="<c:url value="/docente/imprimirEvaluacionEstudiantes.fautapo"/>" method="POST" target="_blank">
    <div align="center">
      <input type="submit" value="Imprimir"> Paginacion
      <input type="text" name="paginacion" size="3" />
      <div>
        <br>
          ...
          <tr>
            ...
            <c:forEach var="l" items="${lListarFases}" varStatus="contador">
              <c:if test="${l.id_fase < 1000 && l.id_fase <= datosAsignacion.id_fase}">
                ...
                <th <c:if test="${cols > 0}"> colspan="${cols+1+cols_f1}" </c:if> >
              <c:out value="${l.fase}" />
              </th>
              </c:if>
            </c:forEach>
          ...
          <tr>
            ...
            <c:forEach var="l" items="${lListarFases}" varStatus="contador">
              ...
              <c:if test="${l.id_fase < 1000  && l.id_fase <= datosAsignacion.id_fase}">
                ...
                <th <c:if test="${cols_f > 0}"> colspan="${cols_f}" </c:if> >NOTA
              <br>EVALUADA</th>
              <th <c:if test="${cols_f > 0}"> colspan="${cols_f1}" </c:if> >NOTA
                <br>PONDERADA PROMEDIADA</th>
              <th>NOTA
                <br>FINAL</th>
              </c:if>
            </c:forEach>
          ...
          <tr>
            ...
            <c:forEach var="l" items="${lListarFases}" varStatus="contador">
              ...
              <c:if test="${l.id_fase < 1000 && l.id_fase <= datosAsignacion.id_fase}">
                ...
                <c:forEach var="l1" items="${lFasesTiposNotas}">
                  <c:if test="${l1.id_fase == l.id_fase}">
                    <th colspan="${l1.cantidad}">
                      <c:out value="${l1.tipo_nota}" />
                    </th>
                    <!--
	...
	-->
              ...
              <c:if test="${l.id_fase < 1000 && l.id_fase <= datosAsignacion.id_fase}">
                ...
                <c:forEach var="l1" items="${lFasesTiposNotas}">
                  <c:if test="${l1.id_fase == l.id_fase}">
                    <c:forEach var="nro_nota" begin="1" end="${l1.cantidad}">
                      <th>
                        <c:out value="${nro_nota}" />
                        <br>
                        <input type="checkbox" checked="checked" name="datos_impresion" value="${nro_nota}/${l1.id_tipo_nota}/${l1.id_fase}/1">
                      </th>
                      <c:set var="ban3" value="1" />
                    </c:forEach>
                  </c:if>
                  ...
                  <c:if test="${l1.id_fase == l.id_fase}">
                    <c:forEach var="nro_nota" begin="1" end="1">
                      <th>
                        <c:out value="${nro_nota}" />
                        <br>
                        <input type="checkbox" checked="checked" name="datos_impresion" value="${nro_nota}/${l1.id_tipo_nota}/${l1.id_fase}/4">
                      </th>
                      <c:set var="ban8" value="1" />
                    </c:forEach>
                  </c:if>
                    ...
                    <c:forEach var="nro_nota" begin="1" end="1">
                      <c:if test="${fase_aux == l1.id_fase}">
                        <th>
                          <c:out value="${nro_nota}" />
                          <br>
                          <input type="checkbox" checked="checked" name="datos_impresion" value="${nro_nota}/${l1.id_tipo_nota}/${l1.id_fase}/3">
                        </th>
                        <c:set var="ban8" value="1" />
                      </c:if>
          ...
          <c:forEach var="lestudiante" items="${lEstudiantes}" varStatus="contador">
            <tr>
              ...
              <td>
                <c:out value="${lestudiante.id_estudiante}" />
              </td>
              <td>
                <c:out value="${lestudiante.nombres}" />
              </td>
                      ...
                      <c:forEach var="nro_nota" begin="1" end="1">
                        ...
                        <c:forEach var="lnota" items="${lEstudiantesEvaluacion}" varStatus="contador1">
                          <c:if test="${l1.id_fase==lnota.id_fase && lnota.id_tipo_nota==l1.id_tipo_nota && lnota.nro_nota==nro_nota && lestudiante.id_estudiante==lnota.id_estudiante}">
                            <c:set var="cols" value="1" />
                            <td align="center">
                              <c:out value="${lnota.nota_final_ponderada}" />
                              <c:set var="suma" value="${suma+lnota.nota_final_ponderada}" />
                            </td>
                          </c:if>
                        </c:forEach>
                        <c:if test="${cols==0}">
                          <td> <c:set var="suma" value="${suma+0}" /></td>
                        </c:if>
                      </c:forEach>
