...
<c:if test="${!empty id_rol}">
  <div class=titulo>Ver Reporte Acta Pre-Cierre</div>
  <br>
  <tabla>
    <form name=formavolver<c:out value="${contador.count}"/> method=post action='<c:url value="/actaPreCierre/listarDepartamentosTiposEvaluaciones.fautapo"/>'>
      <td>
  ...
  <c:if test="${ !empty lAsignacionDocentesProgramaPlan}">
	...
	<td>
        ...
        <form name='forma<c:out value="${contador.count}"/>' method='post' action="<c:url value='/actaPreCierre/listarActaCalificaciones.fautapo'/>" target="_blank">
          <td align="center">     
	    <c:if test="${(asignacion.id_fase == '1000') || (asignacion.id_fase == '7000')}">
