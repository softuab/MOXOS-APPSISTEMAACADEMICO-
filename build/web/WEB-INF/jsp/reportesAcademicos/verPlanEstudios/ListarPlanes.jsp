...
<form name="fvolver" method="POST" action="../planes/verPlanesEstudiosEntrada.fautapo">
...
<c:if test="${fn:length(acceso.listaPlanes) > 0}">
  ...
  <c:forEach var="plan" items="${acceso.listaPlanes}" varStatus="pln">
    combo[h][<c:out value="${pln.index}"/>] = new Array("<c:out value="${plan.id_plan}"/>", "<c:out value="${plan.id_plan}"/>", "<c:out value="${plan.id_programa}"/>");
  </c:forEach>
  h++;
</c:if>
