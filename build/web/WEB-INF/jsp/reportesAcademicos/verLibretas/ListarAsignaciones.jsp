...
<c:if test="${!empty id_rol}">
  ...
  <br>
  <tabla>
    <form name=formavolver<c:out value="${contador.count}"/> method=post action='<c:url value="/verLibreta.fautapo"/>'>
      <td>
  ...
  <c:if test="${ !empty datosAsignacion}">
	...
	<c:if test="${asignacion.id_modelo_ahorro <= 0}">
	  <td align="center">
        ...
        <form name='forma<c:out value="${contador.count}"/>' method='post' action="<c:url value='/listarLibretaEstudiantes.fautapo'/>" target="_blank">
          <td align="center">     
