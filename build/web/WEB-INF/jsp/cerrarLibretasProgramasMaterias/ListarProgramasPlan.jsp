...
<div class="titulo">Cerrar Libretas Programa Materia </div>
<br>
<tabla>
  <form name=formavolver<c:out value="${contador.count}"/> method=post action='<c:url value="/cerrarLibretasProgramasMaterias/entrada.fautapo"/>'>
    <td>
...
<br>
<body onLoad='iniciar()'>
<form name='forma' method="post" action='<c:url value="/cerrarLibretasProgramasMaterias/listarMateriasProgramaPlan.fautapo"/>'>
  <input type="hidden" name="gestion" value='<c:out value="${gestion}"/>' >
  <input type="hidden" name="periodo" value='<c:out value="${periodo}"/>' >
  <input type="hidden" name="id_programa1" value='<c:out value="${cliente.id_programa}"/>' >
...
<c:if test="${fn:length(acceso.listaPlanes) > 0}">
  ...
  <c:forEach var="plan" items="${acceso.listaPlanes}" varStatus="pln">
    combo[h][<c:out value="${pln.index}"/>] = new Array("<c:out value="${plan.id_prg_plan}"/>", "<c:out value="${plan.id_plan}"/> - <c:out value="${plan.tipo_grado}"/>", "<c:out value="${plan.id_programa}"/>");
  </c:forEach>
  h++;
</c:if>
