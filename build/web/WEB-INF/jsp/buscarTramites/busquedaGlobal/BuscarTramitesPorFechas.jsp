...
<c:if test="${accion == 'Buscar'}">
  <div><a class="volver" href='reporteGlobalUbicacionOrganica.fautapo'> Volver</a></div>
  <br>
  <table class="tabla">
    <tr>
      <th>Periodo de</th>
  ...
  <table class="tabla"> 
    ...
    <!-- ********** Esto es para el efecto ************ -->
     ...
     <!-- ********** Fin  efecto ************ -->
     <form name='forma<c:out value="${contador.index}"/>' method="post" action='<c:url value="/listarTramitesGlobales.fautapo"/>' >
       <td>
  ...
  <table class="tabla"> 
    ...
    <c:forEach var="cod" items="${lTramitesMovidos}" varStatus="contador" >
     ...
     <!-- ********** Fin  efecto ************ -->
     <form name='formaM<c:out value="${contador.index}"/>' method=post action='<c:url value="/listarTramitesGlobales.fautapo"/>' >
         <td>
  ...
  <table class="tabla"> 
    ...
    <c:forEach var="cod" items="${lTramitesConcluidos}" varStatus="contador" >
     <!-- ********** Esto es para el efecto ************ -->
       <tr <c:if test="${(contador.count mod 2) == 0}">bgColor="#FFFFD9" %-- Est&acute;tico :( --%</c:if> onMouseOver="this.className='sobreFila'" onmouseout="this.className=''">
     <!-- ********** Fin  efecto ************ -->
      <form name='formaC<c:out value="${contador.index}"/>' method=post action='<c:url value="/listarTramitesGlobales.fautapo"/>' >
          <td>
