...
public class GrabarNotasNoMatriculadas implements Controller {
  ...
  public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    if(accion == null){
      if (null == this.mi.getComprobarUsuario(usuario)) {
        return new ModelAndView("moverNotasMatriculas/MoverNotasNoMatriculadas", "cliente", cliente);
      }
    }else{
	...
	Notas registro = this.mi.getMtcMoverNoMatriculados(mover);
        mensaje="Se registro correctamente";
	modelo.put("mensaje", mensaje);
	return new ModelAndView("moverNotasMatriculas/ConfirmarNotasNoMatriculadas", modelo);
      }
    }
    ...
    mensaje="Esta seguro que desea Mover las notas no matriculadas de la";
    modelo.put("mensaje", mensaje);
    return new ModelAndView("moverNotasMatriculas/GrabarNotasNoMatriculadas", modelo);
  }
}