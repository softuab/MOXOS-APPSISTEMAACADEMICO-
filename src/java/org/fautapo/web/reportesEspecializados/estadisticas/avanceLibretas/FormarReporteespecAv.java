package org.fautapo.web.reportesEspecializados.estadisticas.avanceLibretas;
...
public class FormarReporteespecAv implements Controller {
    ...
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        sql += "\n SELECT * FROM mi_rep_avance_libreta(" + f.sRequest("id_programa") + ", '" + datosPrgPlan.getId_plan() + "', " + datosPrgPlan.getId_tipo_grado() + ", " + f.sRequest("gestion") + ", " + f.sRequest("periodo") + ", " + f.sRequest("id_tipo_evaluacion") + ")";
        sql += "\n AS (id_materia integer, nivel_academico dentero, sigla dtexto, materia dtexto, fase dtexto, grupo dtexto, paterno dtexto2, materno dtexto2, nombres dtexto)";

        System.out.println("\n\n--Listar Transacciones-->\n" + sql + ";\n\n\n");

        //definicion de las etiquetas
        String etiquetas = "Idmateria###Nivel###Sigla###Materia###Fase###Grupo###Paterno###Materno###Nombres";

        //Generacion de la matriz
        f.matriz_etiquetas(sql, "datos", 1, "desde", 9, etiquetas);
        modelo.put("nombres", cliente.getNombres());

        modelo.put("periodo", f.sRequest("periodo"));
        modelo.put("gestion", f.sRequest("gestion"));

        return new ModelAndView("reportesEspecializados/estadisticas/avanceLibretas/FormarReporte", modelo);
    }
}
