...
/////////////////////CALENDARIO
            int tiponota = datosTipoNota.getId_tipo_nota();
            int id_materia = datosAsignacion.getId_materia();
            int id_grupo = datosAsignacion.getId_grupo();

            String mensajeerror = null;
            if (sNro_nota_s != null) {
                if (tiponota == 3 || tiponota == 5) {
                    ...
                    Libretas calendario = new Libretas();
                    calendario.setId_programa(datosPrograma.getId_programa());
                    calendario.setGestion(datosAsignacion.getGestion());
                    calendario.setPeriodo(datosAsignacion.getPeriodo());
                    calendario.setfec_actual(fechaSis);
                    calendario.setId_tipo_nota(datosTipoNota.getId_tipo_nota());
                    calendario.setId_tipo_evaluacion(datosAsignacion.getId_tipo_evaluacion());
                    calendario.setnro_tipo_nota(Integer.parseInt(sNro_nota_s));
                    int iValor = this.mi.setBuscarCalendarioAcademicoPrograma(calendario);

                    System.out.println("ENTROOO------->" + tiponota);
                    System.out.println("PROGRAMA------->" + datosPrograma.getId_programa());
                    System.out.println("GESTION------->" + datosAsignacion.getGestion());
                    System.out.println("PERIODO------->" + datosAsignacion.getPeriodo());
                    System.out.println("TIPO NOTA------->" + datosTipoNota.getId_tipo_nota());
                    System.out.println("TIPO EVAL------->" + datosAsignacion.getId_tipo_evaluacion());
                    System.out.println("FECHASIS------->" + fechaSis);
                    System.out.println("VALOR------->" + iValor);
                    System.out.println("NRO NOTA------->" + sNro_nota_s);

                    Materias excepcion = new Materias();
                    excepcion.setId_materia(datosAsignacion.getId_materia());
                    int id_excepcion = this.mi.getBuscar_nro_excepcion_calendario(excepcion);
                    System.out.println("programa------->" + datosPrograma.getId_programa());
                    System.out.println("id_excepcion------->" + id_excepcion);

                    Libretas calendario1 = new Libretas();
                    calendario1.setId_programa(datosPrograma.getId_programa());
                    calendario1.setGestion(datosAsignacion.getGestion());
                    calendario1.setPeriodo(datosAsignacion.getPeriodo());
                    calendario1.setfec_actual(fechaSis);
                    calendario1.setId_tipo_nota(datosTipoNota.getId_tipo_nota());
                    calendario1.setId_tipo_evaluacion(datosAsignacion.getId_tipo_evaluacion());
                    calendario1.setnro_tipo_nota(Integer.parseInt(sNro_nota_s));
                    calendario1.setId_materia(id_materia);
                    calendario1.setId_grupo(id_grupo);
                    int ampliacion = this.mi.getBuscar_ampliacion_calendario(calendario1);

                    if (iValor != 1) {
                        if (ampliacion != 1) {

                            if (id_excepcion != 1) {
                                mensajeerror = "CALIFICACION FUERA DE FECHA O NO REGISTRADO EN EL CALENDARIO ACADEMICO";
                                //modelo.put("iValor", Integer.toString(iValor));
                                modelo.put("mensajeerror", mensajeerror);
                                return new ModelAndView("administrarLibretas/ListarEstudiantesProgramados", modelo);
                            }
                        }
                    }
                }
