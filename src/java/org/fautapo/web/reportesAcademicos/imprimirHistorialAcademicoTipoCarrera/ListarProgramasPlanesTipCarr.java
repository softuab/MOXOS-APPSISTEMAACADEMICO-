 ...
 */
public class ListarProgramasPlanesTipCarr implements Controller {
    ...
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (null == this.mi.getComprobarUsuario(usuario)) {
            return new ModelAndView("reportesAcademicos/imprimirHistorialAcademicoTipoCarrera/Entrada", "cliente", cliente);
        }
        modelo.put("cliente", cliente);
        modelo.put("acceso", (Accesos) request.getSession().getAttribute("__sess_acceso"));

        return new ModelAndView("reportesAcademicos/imprimirHistorialAcademicoTipoCarrera/ListarProgramasPlanes", modelo);
    }
}
