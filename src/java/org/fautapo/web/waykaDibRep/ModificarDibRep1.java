...
public class ModificarDibRep1 implements Controller {
  ...
  //LISTA A LOS QUE ENTRAN A MOSTRAR CON COUNT()
    ...
    if(!sql_total.equals("")){
      sql_total ="SELECT (" + sql_total + ")::text as valores FROM " + sql_tablas; 
      if (!"".equals(sql_condiciones)){
        sql_total = sql_total + " WHERE " + sql_condiciones;
    ...
    sql = "SELECT " + sql_campos2 + " FROM " + sql_tablas;

    if (!"".equals(sql_condiciones)){
      sql = sql + " WHERE " + sql_condiciones;
      if (!sql_variables.equals("")){
      sql = sql + " ## ";
      }
    } else { 
