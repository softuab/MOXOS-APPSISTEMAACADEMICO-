...
public class RegistrarClave implements Controller {
  ...
  public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    if ("Aceptar".equals(sBoton)) {
      ...
      if (((sNuevaClave.trim()).length() >= 6) && ((sNuevaClave.trim()).length() <= 10)) {
        ...
        if (iResultado == 0) {
          return new ModelAndView("Error", "mensaje", "La nueva clave no se registro");
        }
        else {
          return new ModelAndView("Aviso", "mensaje", "La clave fu� registrada correctamente");
        }
      }
      else {
        return new ModelAndView("Error", "mensaje", "La nueva clave debe tener un m�nimo de 6 caracteres y un m�ximo de 10");
      }
    }
