...
<body onLoad='iniciar()'>
<form name="formavolver" method=post action='<c:url value="/administrarTiposNotas/entrada.fautapo"/>'>
      <div><a class="volver" href="javascript:document.formavolver.submit();"> Volver</a></div>
</form>

<form name='forma' method="post" action='<c:url value="/administrarTiposNotas/listarLbrTiposNotas.fautapo"/>'>
...
<table class="formulario">
  ...
  <c:if test="${fn:length(acceso.listaDepartamentos) > 0}">
      <tr>
        <td class="etiqueta">Departamentos<font color='red'>(*)</font></td>
        <td class="etiqueta">::</td>
...
<c:if test="${fn:length(acceso.listaDepartamentos) > 0}">
  ...
  combo[h] = new Array();
  <c:forEach var="departamento" items="${acceso.listaDepartamentos}" varStatus="dpt">
    combo[h][<c:out value="${dpt.index}"/>] = new Array("<c:out value="${departamento.id_departamento}"/>", "<c:out value="${departamento.departamento}"/>", "<c:out value="${departamento.id_facultad}"/>");
  </c:forEach>
  h++;
</c:if>
