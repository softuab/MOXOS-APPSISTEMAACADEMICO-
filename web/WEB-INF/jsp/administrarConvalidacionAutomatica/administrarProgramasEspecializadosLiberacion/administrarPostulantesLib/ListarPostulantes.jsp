...
<table>
  <tr>
    <td>
     <form name="fvolver" action="<c:url value='/postulantesLib/entrada.fautapo'/>" method="post">
    ...
    <td>
     <form name="fnuevo" action="<c:url value='/postulantesLib/nuevoPostulante.fautapo'/>" method="post">
      <input type="hidden" name="id_proceso" value="<c:out value='${id_proceso}'/>">
      <input type="hidden" name="id_tramite" value="<c:out value='${id_tramite}'/>">
      <input type="hidden" name="titulo" value="<c:out value='${titulo}'/>">
...
<div class="centro">
  ...
  <c:forEach var="lista" items="${lPstPersonas}" varStatus="contador">
    <tr <c:if test="${(contador.count mod 2) == 0}">bgColor="#FFFFD9" %-- Est&acute;tico :( --%</c:if> onMouseOver="this.className='sobreFila'" onmouseout="this.className=''">
      ...
      <td class="etiqueta">
        <form name="fregistra<c:out value='${contador.count}'/>" action="<c:url value='/postulantesLib/nuevoPrsPostulante.fautapo'/>" method="post">
          <input type="hidden" name="id_proceso" value="<c:out value='${id_proceso}'/>">
          <input type="hidden" name="id_tramite" value="<c:out value='${id_tramite}'/>">
          <input type="hidden" name="titulo" value="<c:out value='${titulo}'/>">
