...
<br>
<form  name=forma action="<c:url value="/estudianteAntiguoDocumentos/registrarCompromisosDocumentosPersona.fautapo"/>" method="POST">
	 ...
	 <table class="tabla">
	   ...
	   <c:forEach var="lCompromiso" items="${lTiposCompromisos}" varStatus="contador">
	     <tr <c:if test="${(contador.count mod 2) == 0}">bgColor="#FFFFD9" %-- Est&acute;tico :( --%</c:if> onMouseOver="this.className='sobreFila'" onmouseout="this.className=''">
	       ...
	       <td> 
	         <input type="text" name="fec_vencimiento<c:out value='${lCompromiso.id_tipo_compromiso}'/>"  value='<fmt:formatDate value="${now}" pattern="yyyy-MM-dd"/>'
