	...
	<td width="0%" align=center>
		...
		<td  width = "85%"  align="center">
			...
			<table  width="100%" heigth="10%" cellpading ="1" cellspacing ="0" border = 0 >
				...
				<br>
				<br>
				<td height= "100%"  width ="50%" align ="center"><font size="3">CERTIFICADOS ANULADOS</font></td>
...
<c:if test="${(contador.count) == 1}">
	<table width = "90%" height= "20%" border = 0 >
		...
		<tr>
        <td align="lefth"><label>Certificados anulados:: Del <c:out value='${fec_comprobantei}'/> Al  <c:out value='${fec_comprobantef}'/></label></td>
        </tr>
	</table>
...
<br>
  ...
  <tr>
	...
	<th><font size="2">Fecha Anulacion</font></th>
  </tr>
  <c:set var="x" value="0"/>
  <c:set var="y" value="0"/>
  <c:forEach var="lista" items="${lDatosAnuladosCertificados}" varStatus="contador">
	<c:if test="${(lista.facultad!=y)}">
	<td colspan = "9"><font size="2"><B>
				  <c:out value="${lista.facultad}"/></B></font></td>
    </c:if>
	<c:if test="${(lista.carrera!=x)}">
	<tr>
	<td></td>
	<td colspan = "8"><font size="2"><B><c:out value="${lista.carrera}"/></B></font></td>
	</tr>
	</c:if>
	<tr>
	<td></td>
	...
	<td><font size="2"><fmt:formatDate value="${lista.fec_modificacion}" pattern="yyyy-MM-dd"/></font></td>
    </tr>
