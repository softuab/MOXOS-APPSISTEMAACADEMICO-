...
<div class="titulo">Estudiantes Matriculados con Tipos de Descuentos</div>
<div class="volver"><a href='entrada.fautapo'>Volver</a></div>

<body onLoad='iniciar()'>
<form name='forma' method="post" action="listarEstudiantes.fautapo" target="_blank">
...
<table class="formulario">
  ...
  <c:if test="${fn:length(acceso.listaProgramas) > 0}">
    ...
    <tr>
      <td class="etiqueta">Tipo de Descuentos <font color="red">(*)</font></td>
      <td class="etiqueta">::</td>
      <td>
        <select name="id_tipo_descuento">
         <option value=""> - Elija una opción - </option>    
          <c:forEach var="tipo" items="${lTiposDescuentos}">
            <option value="<c:out value="${tipo.id_tipo_descuento}"/>"> <c:out value="${tipo.tipo_descuento}"/></option>
          </c:forEach>
        </select>
      </td>
    </tr>
