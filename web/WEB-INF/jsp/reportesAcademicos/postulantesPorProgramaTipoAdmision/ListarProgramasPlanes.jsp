...
<div class="titulo">Postulantes Inscritos Por Programa y Tipo Admisi&oacute;n</div>
<div class="volver"><a href='entrada.fautapo'>Volver</a></div>

<body onLoad='iniciar()'>
<form name='forma' method="post" action="listarPostulantes.fautapo" target="_blank">
...
<table class="formulario">
  ...
  <c:if test="${fn:length(acceso.listaProgramas) > 0}">
    ...
    <tr>
      <td class="etiqueta">Tipo de Admision <font color="red">(*)</font></td>
      <td class="etiqueta">::</td>
      <td>
        <select name="id_tipo_admision">
         <option value=""> - Elija una opción - </option>    
          <c:forEach var="tipo" items="${lTiposAdmisiones}">
            <option value="<c:out value="${tipo.id_tipo_admision}"/>"> <c:out value="${tipo.tipo_admision}"/></option>
          </c:forEach>
        </select>
      </td>
    </tr>
...
<center><br>
    <input type=submit name='boton' value='Inscritos'>
    <input type=submit name='boton' value='Aprobados'>
    <input type=submit name='boton' value='Reprobados'><br>
</center>
