...
<div><a class="volver" href="<c:url value="/buscarTramiteEstado.fautapo?"></c:url> ">Volver</a></div>
<br>
...
<table class="tabla">
  ...
  <c:forEach var="cod" items="${lTramites}" varStatus="contador">
     <!-- ********** Esto es para el efecto ************ -->
       <tr <c:if test="${(contador.count mod 2) == 0}">bgColor="#FFFFD9" %-- Est&acute;tico :( --%</c:if> onMouseOver="this.className='sobreFila'" onmouseout="this.className=''">
     <!-- ********** Fin  efecto ************ -->
    <form name='forma<c:out value="${contador.index}"/>' method=post action='<c:url value="/listarDetalleTramite.fautapo"/>' >
      ...
      <input type="hidden" name="id_tipo_proceso" value='<c:out value="${cod.id_tipo_proceso}"/>' >
    </form>
  </tr>
  </c:forEach>
</table>
